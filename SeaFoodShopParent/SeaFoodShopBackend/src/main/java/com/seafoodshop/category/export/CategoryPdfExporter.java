package com.seafoodshop.category.export;
...
public class CategoryPdfExporter extends AbstractExporter {
	...
	super.setResponseHeader(response, "application/pdf", ".pdf", "categories_");
		...
		table.setWidths(new float[] {1.2f, 3.0f, 3.0f, 1.7f, 3.0f});
	...
	private void writeTableData(PdfPTable table, List<Category> listCategories) {
		...
		for (Category category : listCategories) {
			...
			table.addCell(category.getCategoryNameParent());
	...
	private void writeTableHeader(PdfPTable table) {
		...
		cell.setPhrase(new Phrase("Category Name Parent", font));        
        table.addCell(cell);
